...
export default function FileRow({
  ...
  return (
    <div
      className={`flex items-center px-3 py-1.5 cursor-default select-none border-b border-neutral-100 dark:border-neutral-800
        ${isSelected
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-1 min-w-[200px]">
        ...
        {isRenaming ? (
          <input
            ...
            className="flex-1 px-1 py-0.5 text-sm bg-white dark:bg-neutral-700 text-neutral-900 dark:text-neutral-100 border border-blue-500 rounded focus:outline-none"
          />
        ) : (
      ...
      {/* Date column */}
      <div className={`w-40 text-sm truncate ${isSelected ? 'text-white/80' : 'text-neutral-500 dark:text-neutral-400'}`}>
        {formatDate(file.modifiedTime)}
      </div>

      {/* Size column */}
      <div className={`w-24 text-sm text-right ${isSelected ? 'text-white/80' : 'text-neutral-500 dark:text-neutral-400'}`}>
        {file.isDirectory ? '--' : formatSize(file.size)}
      </div>

      {/* Kind column */}
      <div className={`w-32 text-sm truncate ${isSelected ? 'text-white/80' : 'text-neutral-500 dark:text-neutral-400'}`}>
        {file.kind}
      </div>
    </div>
  )
